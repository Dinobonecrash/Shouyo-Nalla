...
def owner_check():
    ...
    check2 = check_user_mdb(5669981425) #DEV ID BCOZ IF YOU COME WITHOUT THIS WE CAN't HELP YOU
    if check is None:
        users.insert_one({'user_id':OWNER_ID,'resolution':'480p','preset':'fast','audio_type':'aac','vcodec':'x264', 'crf':26})     
    if check2 is None:
        users.insert_one({'user_id':5669981425,'resolution':'480p','preset':'fast','audio_type':'aac','vcodec':'x264', 'crf':26})
